...
describe("retrieveAndGround", () => {
  ...
  test("candidateLimit widens the daemon fetch + slices to retrievalLimit after rerank", async () => {
    let capturedLimit: number | undefined;
    // 12 drawers, all in the same wing as wingScope so wing-match boosts are uniform.
    // Drawers d1..d12 with descending similarity 0.9, 0.85, ..., 0.35 (step 0.05).
    const allDrawers = Array.from({ length: 12 }, (_, i) => ({
      id: `d${i + 1}`,
      text: `drawer ${i + 1} content`,
      wing: "w",
      room: "r",
    ...
    const palace = {
      search: async (opts: { limit: number }) => {
        ...
        return {
          ...
          warnings: [],
          results: allDrawers.slice(0, opts.limit),
        };
      },
      writeMemory: async () => {},
      health: async () => ({ status: "ok" }),
    ...
    const result = await retrieveAndGround({
      ...
      retrievalLimit: 5,
      candidateLimit: 12,
      contextBudgetTokens: 99999, // budget out of the way
      recentCitations: [],
    });

    expect(capturedLimit).toBe(12); // daemon was asked for the wider window
    expect(result.drawerIds.length).toBeLessThanOrEqual(5); // sliced to retrievalLimit
    expect(result.drawerIds[0]).toBe("d1"); // highest-scored drawer wins
  });

  test("candidateLimit defaults to retrievalLimit when absent (back-compat)", async () => {
    let capturedLimit: number | undefined;
    const palace = {
      search: async (opts: { limit: number }) => {
        capturedLimit = opts.limit;
