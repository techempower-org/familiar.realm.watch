...
import type { PalaceClient, WriteMemoryOpts, WriteMemoryResult } from "../../src/palace-client.ts";
...
const EMPTY_WRITE_RESULT: WriteMemoryResult = { id: "", warnings: [], errors: [] };

function stubPalace(
  searchResult: PalaceDrawer | undefined,
  writeSpy?: (opts: WriteMemoryOpts) => Promise<WriteMemoryResult>,
): PalaceClient {
  return {
    ...
    writeMemory: writeSpy ?? (async () => EMPTY_WRITE_RESULT),
...
describe("ReflectWriter.review", () => {
  test("writes facts that pass gate and are novel", async () => {
    ...
    const palace = stubPalace(undefined, async (opts) => { writes.push(opts); return EMPTY_WRITE_RESULT; });
  ...
  test("dedupes against existing palace content", async () => {
    ...
    const writes: WriteMemoryOpts[] = [];
    const palace = stubPalace(top, async (opts) => { writes.push(opts); return EMPTY_WRITE_RESULT; });
    const writer = new ReflectWriter({
      palace,
  ...
  test("propagates daemon warnings + drawer_id onto written decisions (mempalace#86)", async () => {
    const palace = stubPalace(undefined, async () => ({
      id: "drawer_new123",
    ...
    const decisions = await writer.review({ sessionId: "s-warn", assistantTurn: "x" });
    expect(decisions).toHaveLength(1);
    expect(decisions[0].status).toBe("written");
    expect(decisions[0].drawer_id).toBe("drawer_new123");
    expect(decisions[0].warnings).toEqual(["hnsw_index_pending"]);
    expect(decisions[0].errors).toBeUndefined();
  });
